...
describe("assertNonEmptyVariable", () => {
  it("Should not throw an error if the parameter is an object", () => {
    expect(() =>
      assertNonEmptyVariable({ test: "test" }, "Test Empty Variable", null as unknown as string),
    ).not.toThrow();

    expect(() =>
      assertNonEmptyVariable("", "Test Non Empty Variable", null as unknown as string),
